...
int linenoise_read_line(char *s, int timeout) {
	if (timeout) {
		ESP_ERROR_CHECK(esp_timer_start_once(timeout_timer, timeout*1000000));
	}
	memset(s, 0, INPUT_BUFFER_SIZE);
	esp_err_t r=esp_linenoise_get_line(handle, s, INPUT_BUFFER_SIZE-2);
	if (timeout) esp_timer_stop(timeout_timer);
	if (r==ESP_OK)  {
		esp_linenoise_history_add(handle, s);
		add_newline(s);
		return 1;
	} else {
		return 0;
			   ...
			   zchar *continued_line_chars)
{
	...
	for (;;) {
		char *command;
		if (prompt) {
			esp_linenoise_set_prompt(handle, prompt);
		} else {
			char prompt[2]={0};
			prompt[0]=(timeout ? "tTD" : ")>}")[type];
			esp_linenoise_set_prompt(handle, prompt);
		}

		/* Prompt only shows up after user input if we don't flush stdout */
		fflush(stdout);
		if (!linenoise_read_line(s, timeout)) {
			//timeout? return '\w'.
			strcpy(s, "\\w\n");
		}
		...
		if (!strcmp(command, "t")) {
			...
			if (!do_more_prompts)
				...
				for (;;) {
					...
					fflush(stdout);
					esp_linenoise_get_line(handle, s, INPUT_BUFFER_SIZE-1);
					add_newline(s);
					if (!strcmp(s, "q\n"))
						break;
				}
...
//			sprintf(prompt, "Please enter a filename [%s]: ", default_name);
		...
		if (strlen(fullpath) == 0)
			buf = strndup(basename((char*)default_name), MAX_FILE_NAME);
	...
	assert(buf);
	snprintf(file_name, FILENAME_MAX, "/spiffs/%s", buf);
	if (freebuf) free(buf);
